...
def _main():
	...
	parser.add_argument("-us", "--userrow", action="append", nargs="*",
						help="User reg to set (syntax: USERROW_nr:0xvalue)")
	parser.add_argument("-ur", "--readuserrow", action="store_true",
						help="Read out the userrow-registers")
	parser.add_argument("-v", "--verbose", action="store_true",
						help="Set verbose mode")

	args = parser.parse_args(sys.argv[1:])

	if not any( (args.fuses, args.flash, args.erase, args.reset, args.readfuses, args.userrow, args.readuserrow)):
		...
		for fslist in args.fuses:
			for fsarg in fslist:
				...
				if not _set_fuse(nvm, fusenum, value):
					return False
	if args.userrow is not None:
		for uslist in args.userrow:
			for usarg in uslist:
				if not re.match("^[0-9]+:0x[0-9a-fA-F]+$", usarg):
					print("Bad USER ROW format {}. Expected USERROW_nr:0xvalue".format(usarg))
					continue
				lst = usarg.split(":0x")
				urnum = int(lst[0])
				value = int(lst[1], 16)
				if not _set_userrow(nvm, urnum, value):
					return False
...
def _set_userrow(nvm, urnum, value):
	nvm.write_userrow(urnum, value)
	actual_val = nvm.read_userrow(urnum)
	ret = actual_val == value
	if not ret:
		print("Verify error for fuse {0}, expected 0x{1:02X} read 0x{2:02X}".format(urnum, value, actual_val))
	else:
		print("USER ROW {0} set to 0x{1:02X} successfully".format(urnum, value))
	return ret
...
def _read_urow(nvm):
	print("U ROW:Value")
	for urnum in range (0,32): # This range should probably be defined for each chip
		urval=nvm.read_userrow(urnum)
		print("{0}:0x{1:02X}".format(urnum,urval))
	return True
