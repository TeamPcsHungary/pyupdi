...
class UpdiNvmProgrammer(object):
	...
	def write_flash(self, address, data):
		...
		for page in pages:
			i += 1
			print("\r%.02f%%" % (float(len(page) * i) / len(data) * 100), end='')
			self.logger.info("Writing page at 0x{0:04X}".format(address))
			self.application.write_nvm(address, page)
			address += len(page)
		print()

	def _read_at_address(self, block_address, fusenum):
		"""
			Reads one fuse or userrow value
		"""
		# Must be in prog mode
		if not self.progmode:
			raise Exception("Enter progmode first!")

		address = block_address + fusenum
	...
	def read_fuse(self, fusenum):
		...
		"""
		return self._read_at_address(self.device.fuses_address, fusenum)

	def read_userrow(self, datanum):
		"""
			Reads one userrow value
		"""
		return self._read_at_address(self.device.userrow_address, datanum)

	def _write_at_address(self, block_address, fusenum, value):
		"""
			Writes one fuse or userrow value
		"""
		# Must be in prog mode
		if not self.progmode:
			raise Exception("Enter progmode first!")
		...
		fuse_data = [value]
		fuse_address = block_address + fusenum
	...
	def write_fuse(self, fusenum, value):
		self._write_at_address(self.device.fuses_address, fusenum, value)
	
	def write_userrow(self, reg_num, value):
		self._write_at_address(self.device.userrow_address, reg_num, value)
